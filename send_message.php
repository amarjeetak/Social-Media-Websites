<!DOCTYPE html>
<?php
session_start();
include("db/db.php");
include("function/function.php");
?>
<html>
<head>
	<title></title>
	 <meta name="viewport" content="width=device-width, initial-scale=1">
	  
	<link rel="stylesheet" type="text/css" href="style.css">
</head>
<body>
	<div class="maincontent">
	<div id="header">
		<div id="logo">
			MANIT SOCIAL NETWORKING WEBSITE
		</div>
	</div>
	<div class="navbar">
		<?php
		if (isset($_SESSION['email'])) {
			# code...
		
				$email=$_SESSION['email'];
				$user="select * from users where user_email='$email' ";
				$run=mysqli_query($con,$user);
				$row=mysqli_fetch_array($run);
				$user_id=$row['user_id'];
				$user_name=$row['user_name'];
				$desc=$row['desc_user'];
				$user_img=$row['user_img'];
				$_SESSION['user_id']=$user_id;
				//to count the number of posts that user has posted
				$post_query="select * from posts where user_id='$user_id' ";
				$run_posts=mysqli_query($con,$post_query);
				$count_posts=mysqli_num_rows($run_posts);

				//to count the number of unread messages
				$msg_query="select * from message where reciever='$user_id' and status='unread' order by 1 desc ";
				$run_msg=mysqli_query($con,$msg_query);
				$count_msg=mysqli_num_rows($run_msg);
	echo "<ul>
			<li><a href='index.php'>Home</a></li>
			<li><a href='my_profile.php'>Profile</a>
				<ul>
					<li><a href='my_profile.php'>Change Profile</a></li>
					<li><a href='changepassword.php'>Change Password</a></li>
					<li><a href='logout.php'>Log Out</a></li>
				</ul>
			</li>
			<li><a href='#'>Blog</a>
				<ul>
					<li><a href='my_message.php?Inbox&user_id=$user_id'>Message ($count_msg)</a></li>
					<li><a href='my_post.php?user_id=$user_id'>Posts ($count_posts)</a></li>
					<li><a href='#'>Friends</a></li>
				</ul>
			</li>
			<li><a href='friends.php'>All Friends</a></li>
			<li><a href='#'>Contact</a></li>
		
		
			<form method='post' action='#' id='searchbox'>
				<input type='text' name='search' placeholder='Seach...'' />
				<input type='submit' name='search' value='Search'>
			</form>
			</ul>
	</div>
	
		<div id='left'>
			
				<center>
					<img src='images/$user_img'  />
				</center>
				<div id='user_details'>
					<p><center><h2>$user_name</h2></center>
					<center><strong>$desc</strong></center></p>
					</br>
					
				</div>";
			}
			else {
				# code...
				echo "<script>alert('Login First')</script>";
				echo "<script>window.open('login.php','_self')</script>";
			}
			?>
		
		
		</div>
		<div id="content">
			<?php $reciever_id=$_GET['u_id'];
				$query="select * from users where user_id='$reciever_id' ";
				$run=mysqli_query($con,$query);
				$row=mysqli_fetch_array($run);
				$u_img=$row['user_img'];
				$u_name=$row['user_name'];
				echo "
				<p style='margin-top:5px; text-align:center; color:black;font-family:sans MS;font-weight:bold;font-size:20px;'>Send Message to<a href='profile.php?user_id=$reciever_id'> $u_name</a></p>
				<img src='images/$u_img' width='80' height='80' style='float:right;margin-top:0px;margin-right:30px;border:2px solid black;'> 

				";
					
			 ?>
			<form method="post" action="" id="f">
				<textarea cols="50" rows="5" placeholder="Enter something to post ...." name="content"></textarea><br><br>
				<input type="hidden" name="reciever_id" value="<?php echo $reciever_id; ?>">
				<input type="submit" name="post"  value="Send"> 
			</form>
			<?php insertmessage();?>
		</div>
		<div id="right">
			
		</div>
	
		
</div>

</body>
</html>